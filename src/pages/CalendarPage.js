...
const CalendarPage = () => {
  ...
  const loadSlots = useCallback(async (date) => {
    ...
    try {
      ...
      const availableSlots = await api.getSlots(master.id, dateStr);
      ...
      setError('Не удалось загрузить свободное время. Попробуйте позже.');
    } finally {
      setLoading(false);
    }
  }, [master?.id]);
  ...
  const confirmBooking = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Ошибка при записи');
