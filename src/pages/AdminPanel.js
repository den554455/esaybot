...
const AdminMasters = () => {
  ...
  const handleAddMaster = async (e) => {
    e.preventDefault();
    try {
      await api.post('/admin/masters', newMaster);
...
const AdminReviews = () => {
  ...
  const loadReviews = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/admin/reviews?status=${statusFilter}`);
  ...
  const handleUpdateStatus = async (reviewId, status) => {
    try {
      await api.post(`/admin/reviews/${reviewId}`, { status });
      await loadReviews();
...
const AdminFeedback = () => {
  ...
  const handleMarkResponded = async (feedbackId) => {
    try {
      await api.post(`/admin/feedback/${feedbackId}`, { status: 'responded' });
      await loadFeedback();
