...
const MasterSchedulePage = () => {
  ...
  const loadSchedule = async () => {
    try {
      ...
      if (response.data.success) {
        const existing = response.data.schedule || [];
  ...
  const handleSaveSchedule = async () => {
    setSaving(true);
    try {
      await api.post('/master/schedule', { schedule });
  ...
  const handleAddException = async () => {
    ...
    try {
      await api.post('/master/exceptions', newException);
      setNewException({ date: '', is_working: false, reason: '' });
      loadExceptions();
  ...
  const handleRemoveException = async (date) => {
    try {
      await api.delete(`/master/exceptions/${date}`);
      loadExceptions();
